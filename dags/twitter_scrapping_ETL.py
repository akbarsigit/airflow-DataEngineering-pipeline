...
csv_path = Path("/opt/airflow/data/tweet_scrape.csv")
...
def dataScraping():
    ...
    for tweet in tweepy.Cursor(api.search_tweets,tweet_mode="extended", result_type='mixed',q=query, include_entities=True).items(100):
        print(tweet)
        dict_line = {
            "id": str(tweet.id),
...
def dataLoad():
    ...
    cursor = conn.cursor()
    cursor.execute("""
    CREATE TABLE IF NOT EXISTS tweet_scrape(
        id varchar(100), 
        createdAt varchar(100),
        tweetText varchar(350));
    """)
    ...
    with open(csv_path, 'r') as data:
        ...
        for row in reader:
            cursor.execute(
                "INSERT INTO tweet_scrape VALUES (%s, %s, %s)",
                row
            )
    conn.commit()
